...
gulp.task("build:dev", gulp.series("copy:sass:dev", "copy:assets", done => done()));

/**
 * 2.4.2 Prod
 */
// gulp.task("build:prod", gulp.series("copy:php", "copy:sass:prod", "copy:assets", done => done()));
gulp.task("build:prod", gulp.series("copy:sass:prod", "copy:assets", done => done()));
...
gulp.task(
    ...
    gulp.parallel("build:dev", "browser-sync", "file-watcher", done => done())
);
